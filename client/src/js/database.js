...
export const putDb = async (content) => {
  ...
  const floodgate = await openDB(DB_NAME, 1);

  // TODO: Now create a variable for the transaction; again, this will be referenced below.
  const floodgateTransaction = floodgate.transaction(DB_NAME, 'readwrite');

  // TODO: Now create a variable for the store
  const floodgateStore = floodgateTransaction.objectStore(DB_NAME);
...
export const getDb = async () => {
  ...
  const floodgate = await openDB(DB_NAME, 1);
  const floodgateTransaction = floodgate.transaction(DB_NAME, 'readonly');
  const floodgateStore = floodgateTransaction.objectStore(DB_NAME);
